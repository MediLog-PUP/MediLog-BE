...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !isset($_POST['chat_message'])) {
    ...
    if(isset($_POST['first_name'])){
        ...
        $emergency_name = !empty($_POST['emergency_contact_name']) ? $_POST['emergency_contact_name'] : null;
        $emergency_phone = !empty($_POST['emergency_contact_phone']) ? $_POST['emergency_contact_phone'] : null;
        $blood_type = !empty($_POST['blood_type']) ? $_POST['blood_type'] : null;
        $allergies = !empty($_POST['allergies']) ? $_POST['allergies'] : null;
        
        $updateStmt = $pdo->prepare("UPDATE users SET full_name=?, dob=?, gender=?, phone_number=?, address=?, emergency_contact_name=?, emergency_contact_phone=?, blood_type=?, allergies=? WHERE id=?");
        $updateStmt->execute([$full_name, $dob, $gender, $phone_number, $address, $emergency_name, $emergency_phone, $blood_type, $allergies, $user_id]);
    }

    // 2. Handle Profile Picture Upload (Cropped Base64 or standard file fallback)
    if (isset($_POST['cropped_image_data']) && !empty($_POST['cropped_image_data'])) {
        ...
        $new_filename = 'student_' . $user_id . '_' . time() . '.' . $image_type;
    ...
    } else if (isset($_POST['first_name']) && empty($_POST['cropped_image_data'])) {
...
<head>
    ...
    <style>
        ...
        .cropper-view-box, .cropper-face { border-radius: 50%; } /* Circular crop preview */
    </style>
</head>
<body class="font-sans antialiased text-gray-800 bg-gray-50 flex h-screen overflow-hidden">
    ...
    <aside class="hidden md:flex flex-col w-64 bg-gray-900 text-white h-full shadow-xl z-20 flex-shrink-0">
        ...
        <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
            ...
            <a href="student_profile.php" class="flex items-center gap-3 px-4 py-3 bg-pup-maroon text-white rounded-xl font-medium transition-colors shadow-sm"><i data-lucide="user" class="h-5 w-5"></i> Profile</a>
        </nav>
    ...
    <nav class="md:hidden fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 z-50 overflow-x-auto no-scrollbar shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        <div class="flex items-center w-max px-2 min-w-full justify-between">
            ...
            <a href="student_profile.php" class="flex flex-col items-center p-2.5 min-w-[72px] text-pup-maroon transition-colors"><i data-lucide="user" class="h-5 w-5"></i><span class="text-[10px] font-medium mt-1">Profile</span></a>
        </div>
    </nav>
    ...
    <main class="flex-1 flex flex-col h-full overflow-hidden relative">
        <header class="bg-white border-b border-gray-200 px-4 md:px-8 py-4 flex items-center justify-between z-10 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-xl md:text-2xl font-bold text-gray-900">Student Profile</h1>
            </div>
        ...
        <div class="flex-1 overflow-y-auto p-4 sm:p-8 pb-24 md:pb-8">
            <div class="max-w-4xl mx-auto space-y-6">
                ...
                <form id="profileForm" action="student_profile.php" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 sm:p-8">
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-6 mb-8 pb-6 border-b border-gray-100">
                        <div class="relative">
                            ...
                            <!-- Intercept the change event to open the crop modal instead of auto-submitting -->
                    ...
                    <h3 class="text-lg font-bold text-gray-900 mb-4">Academic Information</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
                        ...
                        <div>
                            ...
                            <input type="text" value="<?= htmlspecialchars($user['course'] ?? '') ?>" class="block w-full px-4 py-3 border border-gray-300 rounded-xl bg-gray-50 text-gray-500 sm:text-sm cursor-not-allowed" readonly>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Section</label>
                            <input type="text" value="<?= htmlspecialchars($user['section'] ?? '') ?>" class="block w-full px-4 py-3 border border-gray-300 rounded-xl bg-gray-50 text-gray-500 sm:text-sm cursor-not-allowed" readonly>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
                        ...
                        <div>
                            ...
                            <input type="date" name="dob" value="<?= htmlspecialchars($user['dob'] ?? '') ?>" class="block w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-pup-maroon focus:border-pup-maroon sm:text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Gender</label>
                            <select name="gender" class="block w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-pup-maroon focus:border-pup-maroon sm:text-sm bg-white">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="address" rows="2" class="block w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-pup-maroon focus:border-pup-maroon sm:text-sm"><?= htmlspecialchars($user['address'] ?? '') ?></textarea>
                        </div>
                    </div>

                    <h3 class="text-lg font-bold text-gray-900 mb-4 pt-4 border-t border-gray-100">Medical Data</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Blood Type</label>
                            <select name="blood_type" class="block w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-pup-maroon focus:border-pup-maroon sm:text-sm bg-white">
                                <option value="" <?= empty($user['blood_type']) ? 'selected' : '' ?>>Unknown</option>
                        ...
                        <div>
                            ...
                            <input type="text" name="allergies" value="<?= htmlspecialchars($user['allergies'] ?? '') ?>" placeholder="e.g. Peanuts, Penicillin" class="block w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-pup-maroon focus:border-pup-maroon sm:text-sm">
                        </div>
                    </div>

                    <h3 class="text-lg font-bold text-gray-900 mb-4 pt-4 border-t border-gray-100">Emergency Contact</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Contact Name</label>
                            <input type="text" name="emergency_contact_name" value="<?= htmlspecialchars($user['emergency_contact_name'] ?? '') ?>" class="block w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-pup-maroon focus:border-pup-maroon sm:text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Contact Phone</label>
                            <input type="text" name="emergency_contact_phone" value="<?= htmlspecialchars($user['emergency_contact_phone'] ?? '') ?>" class="block w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-pup-maroon focus:border-pup-maroon sm:text-sm">
                        </div>
                    </div>
    ...
    <script>
        ...
        function openCropModal(event) {
            ...
            if (files && files.length > 0) {
                ...
                reader.onload = function(e) {
                    ...
                    // Initialize cropper after image is loaded into the modal
                    setTimeout(() => {
                        cropper = new Cropper(imageToCrop, {
                            aspectRatio: 1, // Enforce a square crop for profiles
                            viewMode: 1,
                            autoCropArea: 1,
                            dragMode: 'move',
                            guides: false,
        ...
        function closeCropModal() {
            ...
            profilePicInput.value = ''; // Reset the input if canceled
            if (cropper) {
                cropper.destroy();
                cropper = null;
            }
        ...
        function applyCrop() {
            ...
            // Get the cropped canvas with fixed dimensions
            ...
            // Store the data string in the hidden input
            croppedDataInput.value = base64Data;
            
            // Immediately update the UI preview image
            document.getElementById('profileImagePreview').src = base64Data;
            
            // Close the modal
            closeCropModal();
            
            // Automatically submit the form to save the newly cropped image
            profileForm.submit();
        }
    </script>
</body>
