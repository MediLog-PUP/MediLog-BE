...
if (!$active_chat_id && count($students) > 0) {
    $active_chat_id = $students[0]['id']; // Auto select first student if none selected
}
    ...
    <main class="flex-1 flex flex-col md:flex-row h-full overflow-hidden bg-white pb-[72px] md:pb-0">
        ...
        <div class="w-full md:w-1/3 lg:w-1/4 border-r border-gray-200 flex flex-col h-full bg-gray-50">
            <div class="p-4 border-b border-gray-200 bg-white flex items-center justify-between z-10 shadow-sm">
                <h2 class="text-xl font-bold text-gray-900">Student Chats</h2>
                <div class="flex items-center gap-2">
                    ...
                    <button onclick="openLogoutModal()" class="md:hidden text-gray-500 hover:text-red-600 p-2 bg-gray-50 rounded-full border border-gray-200"><i data-lucide="log-out" class="h-5 w-5"></i></button>
                </div>
            </div>
        ...
        <div class="hidden md:flex flex-1 flex-col h-full relative bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-gray-50/50">
            ...
            ?>
            <div class="p-4 border-b border-gray-200 bg-white flex items-center shadow-sm z-10 justify-between">
                ...
                <div class="hidden md:flex items-center gap-3 pl-4 border-l border-gray-200">
                    <img src="<?= $profile_pic ?>" alt="Profile" class="h-8 w-8 rounded-full border-2 border-gray-200 object-cover">
                </div>
            </div>
                    ...
                    <?php if($msg['sender_id'] == $admin_id): ?>
                        <div class="flex justify-end">
                            <div class="bg-gray-800 text-white rounded-2xl rounded-tr-sm px-4 py-3 max-w-[75%] text-sm shadow-md">
                                ...
                                <div class="text-[10px] text-white/60 mt-1 text-right"><?= date("h:i A", strtotime($msg['created_at'])) ?></div>
                            </div>
                        </div>
                    <?php else: ?>
                        <div class="flex justify-start">
                            <div class="bg-white border border-gray-200 text-gray-800 rounded-2xl rounded-tl-sm px-4 py-3 max-w-[75%] text-sm shadow-sm">
                                <?= nl2br(htmlspecialchars($msg['message'])) ?>
                                <div class="text-[10px] text-gray-400 mt-1 text-left"><?= date("h:i A", strtotime($msg['created_at'])) ?></div>
                            </div>
                        </div>
                    <?php endif; ?>
                <?php endforeach; ?>
